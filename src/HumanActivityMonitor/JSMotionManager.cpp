        ...
        JSValueRef* exception)
{
    ...
    try {
        ...
        if(!strcmp(MOTION_ENUM_TYPE_PEDOMETER,contextType.c_str())){
            ...
            JSObjectRef successCB = validator.toFunction(1);
            if (successCB) {
                HRMSuccessCallbackPtr HRMGetCB(new HRMSuccessCallback(GlobalContextManager::getInstance()->getGlobalContext(context)));
                HRMGetCB->setSuccessCallback(successCB);

                // errorCallback
                JSObjectRef errCB = validator.toFunction(2, true);
                if(errCB){
                    HRMGetCB->setErrorCallback(errCB);
                }

                // perform
                MotionManager::getInstance()->getContextInfo(HRMGetCB);
