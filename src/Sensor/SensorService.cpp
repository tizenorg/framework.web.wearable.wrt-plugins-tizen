...
JSValueRef SensorService::getAvailableSensor(JSContextRef context)
{
    ...
    bool bHRMRawGreenSupported = false;
    bool bHRMRawIRSupported = false;
    bool bHRMRawRedSupported = false;
    ...
    ret = sensor_is_supported(SENSOR_HRM_LED_GREEN, &bHRMRawGreenSupported);
    if(ret != SENSOR_ERROR_NONE){
        LOGD("Unknown error");
        SensorUtil::throwSensorException(ret,"hrm green sensor");
    }

    ret = sensor_is_supported(SENSOR_HRM_LED_IR, &bHRMRawIRSupported);
    if(ret != SENSOR_ERROR_NONE){
        LOGD("Unknown error");
        SensorUtil::throwSensorException(ret,"hrm ir sensor");
    }

    ret = sensor_is_supported(SENSOR_HRM_LED_RED, &bHRMRawRedSupported);
    if(ret != SENSOR_ERROR_NONE){
        LOGD("Unknown error");
        SensorUtil::throwSensorException(ret,"hrm red sensor");
    }
    ...
    if (bHRMRawGreenSupported || bHRMRawRedSupported || bHRMRawIRSupported) {
        JSValueRef jsHRMRawType = JSUtil::toJSValueRef(context, hrmrawType);
        if (!JSSetArrayElement(context, jsArray, index, jsHRMRawType)) {
            LOGD("Unknown error");
            throw UnknownException("Unknown error");
        }else{
            index++;
            LOGD("HRM raw Sensor Supported");
        }
    } else {
        LOGW("not supported all hrm raw type");
    }
...
// HRMrawSuccessCallback
HRMRawSuccessCallback::HRMRawSuccessCallback(JSContextRef globalCtx)
    :CallbackUserData(globalCtx)
{
    LOGD("enter");
}

HRMRawSuccessCallback::~HRMRawSuccessCallback()
{
    LOGD("enter");
...
//HRMraw
HRMRawSensor* HRMRawSensor::getInstance(){
    static HRMRawSensor instance;
    return &instance;
}

HRMRawSensor::HRMRawSensor()
{
    int ret = SENSOR_ERROR_NONE;
    LOGD("enter");
    ...
    mHRMSupported = false;
    mGreenSupported = false;
    mRedSupported = false;
    mIrSupported = false;

    ret = sensor_is_supported(SENSOR_HRM, &mHRMSupported);
    if(ret == SENSOR_ERROR_NONE && mHRMSupported == true){
    } else {
        LOGD("not supported HRM type");
    }

    ret = sensor_is_supported(SENSOR_HRM_LED_GREEN, &mGreenSupported);
    if(ret == SENSOR_ERROR_NONE && mGreenSupported == true){
        ret = sensor_get_default_sensor(SENSOR_HRM_LED_GREEN, &mHRMRawGreenHandle);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
            SensorUtil::throwSensorException(ret,"unknown error");
            return;
        }

        ret = sensor_create_listener(mHRMRawGreenHandle, &mHRMRawGreenListener);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
            SensorUtil::throwSensorException(ret,"unknown error");
            return;
        ...
        LOGD("not supported green led type");
    }

    ret = sensor_is_supported(SENSOR_HRM_LED_RED, &mRedSupported);
    if(ret == SENSOR_ERROR_NONE && mRedSupported == true){
        ret = sensor_get_default_sensor(SENSOR_HRM_LED_RED, &mHRMRawRedHandle);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
            SensorUtil::throwSensorException(ret,"unknown error");
            return;
        ...
        ret = sensor_create_listener(mHRMRawRedHandle, &mHRMRawRedListener);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
            SensorUtil::throwSensorException(ret,"unknown error");
            return;
        ...
        LOGD("not supported red led type");
    }

    ret = sensor_is_supported(SENSOR_HRM_LED_IR, &mIrSupported);
    if(ret == SENSOR_ERROR_NONE && mIrSupported == true){
        ret = sensor_get_default_sensor(SENSOR_HRM_LED_IR, &mHRMRawIrHandle);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
            SensorUtil::throwSensorException(ret,"unknown error");
            return;
        ...
        ret = sensor_create_listener(mHRMRawIrHandle, &mHRMRawIrListener);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
            SensorUtil::throwSensorException(ret,"unknown error");
            return;
        ...
        LOGD("not supported ir type");
    }

    if (mHRMSupported || mGreenSupported || mRedSupported || mIrSupported) {
        mBSupported = true;
    } else {
        LOGW("not supported all hrm raw type");
    }
...
HRMRawSensor::~HRMRawSensor()
{
    ...
    if (mGreenSupported) {
        ret = sensor_destroy_listener(mHRMRawGreenListener);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
        }
    }
    ...
    if (mRedSupported) {
        ret = sensor_destroy_listener(mHRMRawRedListener);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
        }
    }
    ...
    if (mIrSupported) {
        ret = sensor_destroy_listener(mHRMRawIrListener);
        if (ret != SENSOR_ERROR_NONE){
            LOGD("unknown error");
        }
    }
...
bool HRMRawSensor::isSupported()
{
    return mBSupported;
}

void HRMRawSensor::sensor_HRMRaw_callback(sensor_h sensor, sensor_event_s *event, void *user_data)
{
    int ret = SENSOR_ERROR_NONE;

    sensor_type_e type = SENSOR_LAST  ;
    ...
    float index = event->values[0];
    LOGD("hrm raw = ", index);
    HRMRawSensor* hrmrawsensor = (HRMRawSensor*)user_data;
    hrmrawsensor->broadcast_HRMRaw(type, index);

}

gboolean HRMRawSensor::callHRMRawStartSuccessCallback(void* data)
{
    LOGD("enter");
    HRMRawSuccessCallbackHolder* holder = static_cast<HRMRawSuccessCallbackHolder*>(data);
    holder->ptr->callSuccessCallback();
    delete holder;
    return false;
}

gboolean HRMRawSensor::callHRMRawSuccessCallback(void* data)
{
    LOGD("enter");
    HRMRawSuccessCallbackHolder* holder = static_cast<HRMRawSuccessCallbackHolder*>(data);
    holder->ptr->callSuccessCallback(holder->mJSHRMRawData);
    JSValueUnprotect(holder->ptr->getContext(), holder->mJSHRMRawData);
    delete holder;
    return false;
}

void HRMRawSensor::broadcast_HRMRaw(sensor_type_e type, float HRMRaw)
{
    bool started = false;
    std::map<JSContextRef, HRMRawSuccessCallbackPtr>::iterator iter;
    int total = mHRMRawCallbackMap.size();

    if(total <= 0){
        return;
    }

    for(iter = mHRMRawCallbackMap.begin() ; iter != mHRMRawCallbackMap.end() ; ++iter){
        started = false;
        started = mHRMRawStartMap[iter->second->getContext()];
        if(true == started){
            convertHRMRawData(type, (unsigned long)HRMRaw, iter->second);
        }
    }
...
void HRMRawSensor::start(HRMRawSuccessCallbackPtr callback)
{
    int ret = 0;
    guint bGIdleRet = 0;
    HRMRawSuccessCallbackHolder* holder = NULL;
    int mapTotal = mHRMRawStartMap.size();

    LOGD("enter");

    if(mapTotal <= 0){
        if (mGreenSupported) {
            ret = sensor_listener_start(mHRMRawGreenListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
                SensorUtil::throwSensorException(ret,"unknown error");
                return;
        ...
        if (mRedSupported) {
            ret = sensor_listener_start(mHRMRawRedListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
                SensorUtil::throwSensorException(ret,"unknown error");
                return;
        ...
        if (mIrSupported) {
            ret = sensor_listener_start(mHRMRawIrListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
                SensorUtil::throwSensorException(ret,"unknown error");
                return;
    ...
    mHRMRawStartMap[callback->getContext()] = true;

    holder = new HRMRawSuccessCallbackHolder();
    if(holder == NULL){
        return;
    }

    holder->ptr = callback;
    holder->mJSHRMRawData = NULL;
    bGIdleRet = g_idle_add(callHRMRawStartSuccessCallback, holder);
    if (!bGIdleRet) {
...
void HRMRawSensor::stop(JSContextRef globalCtx)
{
    LOGD("enter ");
    ...
    mHRMRawStartMap.erase(globalCtx);
    mapTotal = mHRMRawStartMap.size();

    if(mapTotal <= 0){
        if (mGreenSupported) {
            ret = sensor_listener_stop(mHRMRawGreenListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
            }
        }
        ...
        if (mRedSupported) {
            ret = sensor_listener_stop(mHRMRawRedListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
            }
        }
        ...
        if (mIrSupported) {
            ret = sensor_listener_stop(mHRMRawIrListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
            }
        }
...
void HRMRawSensor::setChangeListener(HRMRawSuccessCallbackPtr callback)
{
    LOGD("enter");
    int error = 0;
    int mapTotal = mHRMRawCallbackMap.size();
    std::map<JSContextRef, HRMRawSuccessCallbackPtr>::iterator iter;

    iter = mHRMRawCallbackMap.find(callback->getContext());
    if(iter != mHRMRawCallbackMap.end()){
        mHRMRawCallbackMap.erase(callback->getContext());
        LOGD("prev callback erase");
    }
    mHRMRawCallbackMap[callback->getContext()] = callback;

    if(mapTotal <= 0){
        if (mGreenSupported) {
            error = sensor_listener_set_event_cb(mHRMRawGreenListener, 100, sensor_HRMRaw_callback, this);
            if (error != SENSOR_ERROR_NONE){
                LOGD("unknown error");
                SensorUtil::throwSensorException(error,"unknown error");
                return;
            }
        }

        if (mRedSupported) {
            error = sensor_listener_set_event_cb(mHRMRawRedListener, 100, sensor_HRMRaw_callback, this);
            if (error != SENSOR_ERROR_NONE){
                LOGD("unknown error");
                SensorUtil::throwSensorException(error,"unknown error");
                return;
        ...
        if (mIrSupported) {
            error = sensor_listener_set_event_cb(mHRMRawIrListener, 100, sensor_HRMRaw_callback, this);
            if (error != SENSOR_ERROR_NONE){
                LOGD("unknown error");
                SensorUtil::throwSensorException(error,"unknown error");
                return;
...
void HRMRawSensor::unsetChangeListener(JSContextRef globalCtx)
{
    int total = 0, ret = 0;
    total = mHRMRawCallbackMap.size();
    mHRMRawCallbackMap.erase(globalCtx);

    if(total >= 1){
        if (mGreenSupported) {
            ret = sensor_listener_unset_event_cb(mHRMRawGreenListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
            }
        }
        ...
        if (mRedSupported) {
            ret = sensor_listener_unset_event_cb(mHRMRawRedListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
            }
        }
        ...
        if (mIrSupported) {
            ret = sensor_listener_unset_event_cb(mHRMRawIrListener);
            if (ret != SENSOR_ERROR_NONE){
                LOGD("unknown error");
            }
        }
...
void HRMRawSensor::getSensorData(HRMRawSuccessCallbackPtr callback)
{
    LOGD("enter");
    int ret = 0;
    int error = 0;
    ...
    started = mHRMRawStartMap[callback->getContext()];
    if(false == started){
        mHRMRawStartMap.erase(callback->getContext());
    ...
    if (mGreenSupported) {
        ret = sensor_listener_read_data(mHRMRawGreenListener, &sensorEvent);
        if (ret != SENSOR_ERROR_NONE){
            JSValueRef error = JSWebAPIErrorFactory::makeErrorObject(callback->getContext(), JSWebAPIErrorFactory::SERVICE_NOT_AVAILABLE_ERROR, SensorUtil::getSensorErrorMessage(ret).c_str());
            callback->callErrorCallback(error);
            return;
        }
        HRMRaw = sensorEvent.values[0];

        convertHRMRawData(SENSOR_HRM_LED_GREEN, HRMRaw, callback);
    }

    if (mRedSupported) {
        ret = sensor_listener_read_data(mHRMRawRedListener, &sensorEvent);
        if (ret != SENSOR_ERROR_NONE){
            JSValueRef error = JSWebAPIErrorFactory::makeErrorObject(callback->getContext(), JSWebAPIErrorFactory::SERVICE_NOT_AVAILABLE_ERROR, SensorUtil::getSensorErrorMessage(ret).c_str());
            callback->callErrorCallback(error);
            return;
        ...
        convertHRMRawData(SENSOR_HRM_LED_RED, HRMRaw, callback);
    }

    if (mIrSupported) {
        ret = sensor_listener_read_data(mHRMRawIrListener, &sensorEvent);
        if (ret != SENSOR_ERROR_NONE){
            JSValueRef error = JSWebAPIErrorFactory::makeErrorObject(callback->getContext(), JSWebAPIErrorFactory::SERVICE_NOT_AVAILABLE_ERROR, SensorUtil::getSensorErrorMessage(ret).c_str());
            callback->callErrorCallback(error);
            return;
        ...
        convertHRMRawData(SENSOR_HRM_LED_IR, HRMRaw, callback);
    }

    if ( !mGreenSupported && !mRedSupported && !mIrSupported && mHRMSupported){
        convertHRMRawData(SENSOR_HRM, HRMRaw, callback);
    }

}

void HRMRawSensor::convertHRMRawData(sensor_type_e type, unsigned long HRMRaw, HRMRawSuccessCallbackPtr callback)
{
    guint bGIdleRet = 0;

    HRMRawData* hrmrawdata = NULL;
    HRMRawSuccessCallbackHolder* holder = NULL;

    hrmrawdata = new HRMRawData(type, HRMRaw);
    holder = new HRMRawSuccessCallbackHolder();
    if((hrmrawdata == NULL) || (holder == NULL)){
        return;
    }

    holder->ptr = callback;
    holder->mJSHRMRawData = JSHRMRawData::createJSObject(holder->ptr->getContext(), hrmrawdata);
    JSValueProtect(holder->ptr->getContext(), holder->mJSHRMRawData);

    bGIdleRet = g_idle_add(callHRMRawSuccessCallback, holder);
    if (!bGIdleRet) {
        LOGD("g_idle_add fails");
    }
...
void HRMRawSensor::removeCallback(JSContextRef globalCtx)
{
    LOGD("enter");
