...
// HRM Raw
JSClassDefinition JSHRMRawData::m_classInfo = {
    0,
    kJSClassAttributeNone,
    "HRMRawData",
    NULL, //ParentClass
...
JSStaticValue JSHRMRawData::m_property[] = {
    { TIZEN_HRM_RAW_LIGHT_TYPE, getProperty, NULL, kJSPropertyAttributeReadOnly },
    { TIZEN_HRM_RAW_LIGHT_INTENSITY, getProperty, NULL, kJSPropertyAttributeReadOnly },
    { 0, 0, 0, 0 }
};

JSClassRef JSHRMRawData::m_jsClassRef = JSClassCreate(JSHRMRawData::getClassInfo());

const JSClassRef JSHRMRawData::getClassRef()
{
    if (!m_jsClassRef) {
        m_jsClassRef = JSClassCreate(&m_classInfo);
    }
...
const JSClassDefinition* JSHRMRawData::getClassInfo()
{
    return &m_classInfo;
}


JSValueRef JSHRMRawData::createJSObject(JSContextRef context, HRMRawData* rawdata)
{
    JSObjectRef jsValueRef = JSObjectMake(context, getClassRef(), static_cast<void*>(rawdata));
    if (NULL == jsValueRef) {
        LOGD("object creation error");
        return JSValueMakeUndefined(context);
    }
    return jsValueRef;
}
...
void JSHRMRawData::initialize(JSContextRef context, JSObjectRef object)
{
    LOGD("enter");

}

void JSHRMRawData::finalize(JSObjectRef object)
{
    HRMRawData* priv = static_cast<HRMRawData*>(JSObjectGetPrivate(object));
       if (priv) {
           delete priv;
       }
}

JSValueRef JSHRMRawData::getProperty(JSContextRef context, JSObjectRef object, JSStringRef propertyName, JSValueRef* exception)
{
    ...
    if (JSStringIsEqualToUTF8CString(propertyName, TIZEN_HRM_RAW_LIGHT_INTENSITY)) {
        value = priv->getHRMRaw();
        return JSUtil::toJSValueRef(context, value);
    } else if (JSStringIsEqualToUTF8CString(propertyName, TIZEN_HRM_RAW_LIGHT_TYPE)) {
        std::string type = priv->getHRMLightType();
