...
nfc_se_type_e NFCUtil::toSecureElementType(const std::string &type_string)
{
    ...
    } else if (type_string == HCE) {
        return NFC_SE_TYPE_HCE;
    } else {
...
std::string NFCUtil::toStringHceEventType(
    const nfc_hce_event_type_e type)
{
    switch (type)
    {
        ...
        default:
            LOGE("No Match Card Hce Event Type: %x", type);
            return UNKNOWN_EVENT_RECEIVED;
            //throw Common::TypeMismatchException("No Match Hce Event Type");
    }
}

nfc_card_emulation_category_type_e NFCUtil::toCardEmulationCategoryType(
    const std::string &type_string)
{
    if (type_string == PAYMENT) {
        return NFC_CARD_EMULATION_CATEGORY_PAYMENT;
    } else if (type_string == OTHER) {
        return NFC_CARD_EMULATION_CATEGORY_OTHER;
    } else {
        LOGE("No Match Card Emulation Category Type: %s", type_string.c_str());
        throw Common::TypeMismatchException("No Match Card Emulation Category Type");
    }
}
